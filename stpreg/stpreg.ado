...
program stpreg, eclass byable(onecall)
	version 11.2
	if _by() {
...
*********************************************
* This part and one more further below were modified as interface to stgenreg
* Modified on 2019 09 05
	...
	if inlist(trim("`0'"),"",",") {
		...
		if `: word count `power' `rr' `rd'' > 1 {
			di as error "Can only specify one of rr, rd and power"
			exit 198 
		}
	
		// specify link hazard -> event probability
		if "`rr'" != "" {
			local link "-log(1:-exp([xb]))"
		}
		else if "`power'" != "" {
			local link "exp([xb])"
		}
			...
			local link "log(exp([xb]):+1)"
		}
				
		if "`timefunction'"=="" {
			local xb = cond("`df'"=="","","| #rcs(df(`df') `noorthog' `time')")
			if "`tvc'"!="" {
				foreach x of local tvc {
					local xb2 "`xb2' | `x':*#rcs(df(`=cond("`tvcdf'"=="","`df'","`tvcdf'")') `noorthog' `time')"
				}
			}
		}
		else {
			local xb "| `timefunction'"
			if "`tvc'"!="" {
				if ("`df'"!="" | "`tvcdf'"!="") {
					foreach x of local tvc {
						local xb2 "`xb2' | `x':*#rcs(df(`=cond("`tvcdf'"=="","`df'","`tvcdf'")') `noorthog' `time')"
                    ...
                    foreach x of local tvc {
                        local xb2 "`xb2' | `x':*(`timefunction')"
					}
                }
			}
		}
		local 0 "[`weight'`exp'] `if' `in' , hazard(`link') xb(`varlist' `xb' `xb2') `power' `rr' `rd' `options'"
*********************************************
		`BY' Estimate `0'
...
*********************************************
* Modified on the date indicated at the top
		syntax [fw pw iw aw] [if] [in] 	[,									///
											POWER							/// -Power model
											RR								/// -Risk ratios model
											RD								/// -Risk differences model
											...
											COEF							///	-Display coefficients of first ml equation-
																			///
...
*********************************************
		...
		/* Quadrature nodes and weights */
			...
			forvalues i=1/`ns' {
				...
				stpreg_gaussquad, n(`nnodes`i'') `quadopt`i''
					...
					while `loop'==0 {
						...
						*/
						...
						/* see if #rcs() has been specified */
							...
							stpreg_rcsgen _t, `rcssyntax_`i'_`k'' newgen(_eq`i'_cp`k'_rcs) tousevar(`touse')
						...
						/* Fractional polynomials */
							...
							stpreg_fp `fpsyntax_`i'_`k'' `addcomma_`i'_`k'' stub(_eq`i'_cp`k'_fp) var(_t) tousevar(`touse')
						...
						forvalues sp = 1/`ns' {
								...
								forvalues j = 1/`nnodes`sp'' {
									...
									forvalues k = 1/`ncomp_eqn`i'' {
										...
										if `covidentifier'==1 {
											cap drop _rcs_`k'_*
											qui stpreg_rcsgen `node`j'_`sp'', `time_`i'_`k'' `noorthog_`i'_`k'' newknots(`bhknots_`i'_`k'') newgen(_rcs_`k'_) newrmat(`rmat_`i'_`k'') tousevar(`touse') `addrcsoffset_`i'_`k''
										...
										/* fp's */
											...
											stpreg_fp `fpsyntax_`i'_`k'' `addcomma_`i'_`k'' stub(``node`j''_`sp'_`k'') var(`node`j'_`sp'') tousevar(`touse') //offset included in syntax
		...
		/* Define ml equation syntax */
			...
			cap pr drop stpreg_d0`d0file'
			ml model d0 stpreg_d0`d0file'						///
									`mlequations'				///
									if `touse'					///
									`wt',						///
									`mlopts'					///
			...
			ereturn local cmd stpreg
			ereturn local predict stpreg_pred
...
*********************************************
* Modified on the date indicated at the top
			ereturn local title "Event-probability regression"
*********************************************
...
*********************************************
* Modified on the date indicated at the top
			Replay, level(`level') `coef' `power' `rr' `rd'
*********************************************			
			/* Clear global macros */
...
program Replay
	syntax [, POWER RR RD COEF Level(int `c(level)')]
	if "`rd'" != "" { // display coefficients if rd
		local coef "coef"
	}
	...
	if "`coef'" != "" {
		...
		if "`power'" != "" {
			local eform "eform(Power param.)"
		}
		else if "`rr'" != "" {
			local eform "eform(Risk Ratio)"
		}
		else {
...
program stpreg_rcsgen, eclass
	...
	if "`df'"!="" {
		qui rcsgen `temptime' if `tousevar', df(`df') gen(`newgen') if2(_d==1) `orthog'
		if "`orthog'"!="" {
			tempname matR
			mat `matR' = r(R)
