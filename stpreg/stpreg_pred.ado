...
*********************************************************
	...
	if wordcount(`"`probability' `hazard' `cumhazard' `survival' `failure'"')>1 {
...
* Modified on 2019 02 22
	...
	if "`probability'"!="" {
		...
		qui predictnl `newvarname' = 1 - exp(-(`haz')) if `touse', `opt'
	}
*********************************************************
	...
	if "`cumhazard'"!="" {
		...
		forvalues i=1/`e(ns)' {
			...
			stpreg_gaussquad, n(`e(nodes`i')') `quad' `abopts'
	...
	/* restore original data and merge in new variables */
		...
		if "`probability'`hazard'`cumhazard'`survival'`failure'"!="" {
